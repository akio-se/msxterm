...
また、ターミナルから MSX0 に対しての送信がやたら遅く感じるかもしれませんが、keyboard入力をエミュレートする部分がVsyncか何かでウェイトを入れて同期を取っていて、それが1文字ずつ処理しているため遅いのだと思われます。
ここは MSX0側のシステムの問題なのでいつか改善されることを切に願います。

## まだ未実装の機能
* 入力された文字をターミナル側で可能な限りコード変換をかけてから MSX0側に送信するように対応する予定です。
* MSX0 側から送られてきた MSX ASCII コードをターミナル側の UTF-8 で正しく表示できるように対応する予定です。
